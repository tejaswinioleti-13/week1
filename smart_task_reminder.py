...
class TaskSmartApp(tk.Tk):
    ...
    def _setup_sidebar(self):
        self.sidebar_frame = tk.CTkFrame(self, width=200, corner_radius=0, fg_color=COLORS["sidebar_bg"])
        self.sidebar_frame.grid(row=0, column=0, sticky="nsew")
        self.sidebar_frame.grid_rowconfigure(6, weight=1)

        self.logo_label = tk.TkLabel(self.sidebar_frame, text="TaskSmart", font=ctk.CTkFont(size=20, weight="bold"),
                                       text_color=COLORS["primary"])
        self.logo_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.search_entry = tk.TkEntry(self.sidebar_frame, placeholder_text="Search tasks...")
        self.search_entry.grid(row=1, column=0, padx=20, pady=(10, 20), sticky="ew")

        self.new_task_button = tk.TkButton(self.sidebar_frame, text="➕ New Task", fg_color=COLORS["primary"],
        ...
        self.settings_label = tk.TkLabel(self.sidebar_frame, text="⚙ Settings", text_color=COLORS["secondary_text"])
    ...
    def _create_nav_button(self, text, row, is_active=False):
        ...
        button = tk.TkButton(self.sidebar_frame, text=text, fg_color=fg_color,
    ...
    def _setup_dashboard(self):
        self.dashboard_frame = tk.TkScrollableFrame(self, fg_color=COLORS["dashboard_bg"])
        self.dashboard_frame.grid(row=0, column=1, sticky="nsew", padx=0, pady=0)
        self.dashboard_frame.grid_columnconfigure(0, weight=1)

        # Header
        header_frame = tk.TkFrame(self.dashboard_frame, fg_color="transparent")
        ...
        # Add Task Button
        add_task_btn = tk.TkButton(header_frame, text="➕ Add Task", fg_color=COLORS["danger"], hover_color="#c82333",
                                     width=100, command=self._open_add_task_window)
        add_task_btn.grid(row=0, column=1, rowspan=2, padx=(10, 0), sticky="e")

        # Metric Cards container
        self.metric_card_frame = tk.TkFrame(self.dashboard_frame, fg_color="transparent")
        ...
        self.recent_tasks_container = tk.TkFrame(self.dashboard_frame, fg_color=COLORS["sidebar_bg"], corner_radius=8)
    ...
    def update_dashboard_metrics(self):
        ...
        for i, (title, value, value_color, border_color, progress_type) in enumerate(cards):
            ...
            if progress_type == "progress":
                progress_frame = tk.TkFrame(card, fg_color="transparent")
                progress_frame.grid(row=2, column=0, sticky="ew", padx=15, pady=(5, 5))
                progress_frame.grid_columnconfigure(0, weight=1)

                # Progress Bar
                progress_bar = tk.TkProgressBar(progress_frame, orientation="horizontal", height=8, corner_radius=4)
                ...
                completion_rate_label = tk.TkLabel(card, text=f'{metrics["Completion Rate"]}% Complete',
    ...
    def display_recent_tasks(self):
        ...
        tk.TkLabel(self.recent_tasks_container, text="Recent Tasks", font=ctk.CTkFont(size=18, weight="bold")).grid(
            row=0, column=0, padx=15, pady=(15, 5), sticky="w")

        metrics = self.calculate_metrics()
        summary_frame = tk.TkFrame(self.recent_tasks_container, fg_color="transparent")
        summary_frame.grid(row=1, column=0, padx=15, pady=(0, 10), sticky="w")

        pending_count = metrics["Total Tasks"] - metrics["Completed"]

        summary_text = tk.TkLabel(summary_frame, text=f"{metrics['Total Tasks']} total | {pending_count} pending |",
                                    text_color=COLORS["secondary_text"], anchor="w")
        summary_text.grid(row=0, column=0, sticky="w")

        overdue_text = tk.TkLabel(summary_frame, text=f" {metrics['Overdue']} overdue", text_color=COLORS["danger"],
                                    font=ctk.CTkFont(weight="bold"))
        overdue_text.grid(row=0, column=1, sticky="w")

        # Task List Frame
        task_list_frame = tk.TkFrame(self.recent_tasks_container, fg_color="transparent")
        ...
        tk.TkLabel(task_list_frame, text="Task", font=ctk.CTkFont(weight="bold"),
                     text_color=COLORS["secondary_text"]).grid(row=0, column=0, sticky="w", pady=5)
        tk.TkLabel(task_list_frame, text="Due Date", font=ctk.CTkFont(weight="bold"),
                     text_color=COLORS["secondary_text"]).grid(row=0, column=1, padx=10, sticky="w")
        tk.TkLabel(task_list_frame, text="Status", font=ctk.CTkFont(weight="bold"),
                     text_color=COLORS["secondary_text"]).grid(row=0, column=2, padx=10, sticky="w")
        tk.TkLabel(task_list_frame, text="Complete", font=ctk.CTkFont(weight="bold"),
        ...
        for i, task in enumerate(TASKS):
            ...
            # 1. Task Name
            task_name_label = tk.TkLabel(task_list_frame, text=task["name"], anchor="w")
            task_name_label.grid(row=row, column=0, sticky="w", pady=5, padx=(0, 20))

            # 2. Due Date
            due_date_label = tk.TkLabel(task_list_frame, text=task["due_date"], text_color=COLORS["secondary_text"])
            due_date_label.grid(row=row, column=1, padx=10, sticky="w")

            # 3. Status
            status = task["status"]
            status_color = COLORS.get(status, COLORS["secondary_text"])
            status_label = tk.TkLabel(task_list_frame, text=status.capitalize(), text_color=status_color,
                                        font=tk.TkFont(weight="bold"))
            status_label.grid(row=row, column=2, padx=10, sticky="w")

            # 4. Completion Checkbox
            checkbox = tk.TkCheckBox(
            ...
            # Separator line
            separator = tk.TkFrame(task_list_frame, height=1, fg_color="#e9ecef")
    ...
    def _create_card(self, parent, title, value, value_color=None, border_color=None):
        ...
        card = tk.TkFrame(parent, fg_color=COLORS["sidebar_bg"], corner_radius=8)
        card.grid_columnconfigure(0, weight=1)
        if border_color:
            card.configure(border_width=5, border_color=border_color)

        title_label = tk.TkLabel(card, text=title, text_color=COLORS["secondary_text"], font=ctk.CTkFont(size=14))
        title_label.grid(row=0, column=0, padx=15, pady=(15, 0), sticky="w")

        value_label = tk.TkLabel(card, text=value, font=ctk.CTkFont(size=30, weight="bold"))
        if value_color:
    ...
    def _create_quick_sections(self, parent):
        ...
        section_frame = tk.TkFrame(parent, fg_color="transparent")
        ...
        # --- Quick Overview ---
        quick_frame = tk.TkFrame(section_frame, fg_color=COLORS["sidebar_bg"], corner_radius=8)
        quick_frame.grid(row=0, column=0, padx=(0, 15), sticky="nsew")

        tk.TkLabel(quick_frame, text="Quick Overview", font=tk.TkFont(size=18, weight="bold")).grid(row=0, column=0,
                                                                                                        padx=15,
                                                                                                        pady=(15, 5),
                                                                                                        sticky="w")
        ...
        for i, (label_text, value_text, *color_tuple) in enumerate(overview_details):
            item_frame = tk.TkFrame(quick_frame, fg_color="transparent")
            item_frame.grid(row=i + 1, column=0, padx=15, pady=5, sticky="ew")
            item_frame.grid_columnconfigure(0, weight=1)

            tk.TkLabel(item_frame, text=label_text).grid(row=0, column=0, sticky="w")
            value_label = tk.TkLabel(item_frame, text=value_text, font=tk.TkFont(weight="bold"))
            if color_tuple:
        ...
        activity_frame = tk.TkFrame(section_frame, fg_color=COLORS["sidebar_bg"], corner_radius=8)
        activity_frame.grid(row=0, column=1, padx=(15, 0), sticky="nsew")
        tk.TkLabel(activity_frame, text="Recent Activity", font=tk.TkFont(size=18, weight="bold")).grid(row=0,
                                                                                                            column=0,
                                                                                                            padx=15,
                                                                                                            pady=(15,
                                                                                                                  5),
        ...
        for i, activity in enumerate(activities):
            label = tk.TkLabel(activity_frame, text=f"• {activity}", text_color=COLORS["secondary_text"], anchor="w")
